CREATE OR REPLACE FUNCTION public.admin_dashboard_stats(p_since timestamp with time zone, p_prev_since timestamp with time zone)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'planDist', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('name', COALESCE(plan_type::text,'—'), 'v', c)) FROM (
