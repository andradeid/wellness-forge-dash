...
export default defineTool({
  ...
  description:
    "Retorna um report completo de curadoria a partir do seu id (UUID), incluindo a imagem anexada embutida em base64 (nunca como link), lida do bucket privado curation-attachments no servidor. O campo ai_technical_direction só é incluído para super admins.",
  inputSchema: {
  ...
  handler: async ({ id }, ctx) => {
    try {
      ...
      const { image_url, ...report } = data as unknown as Record<string, unknown>;
      ...
      if (imagePath) {
        try {
          ...
          const { data: file, error: fileError } = await supabaseAdmin.storage
            ...
            .download(imagePath);
          if (fileError || !file) throw new Error(fileError?.message ?? "arquivo indisponível");
          ...
          content.push({
            type: "text",
            text: `Não foi possível carregar a imagem anexada: ${
              imageError instanceof Error ? imageError.message : "erro desconhecido"
            }`,
          });
        }
      }
