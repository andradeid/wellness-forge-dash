...
function TechnicalDirection({ value }) {
    if (!value)
        return <p className="text-muted-foreground">Aguardando análise da IA</p>;
    let parsed = null;
    try {
    ...
    return (<dl className="grid grid-cols-[120px_1fr] gap-x-3 gap-y-1">
      {Object.entries(parsed)
            .filter(([, v]) => v !== null && v !== undefined && String(v).trim() !== "")
            .map(([key, v]) => (<div key={key} className="contents">
...
function AttachmentImage({ path }) {
    ...
    return (<a href={data} target="_blank" rel="noreferrer">
      <img src={data} alt="Imagem anexada à solicitação de curadoria" className="max-h-64 rounded-lg border object-contain transition-opacity hover:opacity-80"/>
    </a>);
}
export function CurationDetailDrawer({ request, curatorName, open, onOpenChange, }) {
      ...
      <SheetContent className="w-full overflow-y-auto sm:max-w-xl">
        {request && (<>
            ...
            <div className="mt-6 space-y-6">
              ...
              {request.image_url && (<section className="space-y-2">
              ...
              <section className="space-y-2">
                ...
                {request.chat_id && (<Button type="button" variant="outline" size="sm" onClick={() => setConversationOpen(true)}>
              ...
              <section className="space-y-2">
                ...
                {request.ai_status === "failed" && (<p className="rounded-md border border-orange-300 bg-orange-50 p-2 text-xs text-orange-900">
                ...
                (request.ai_confidence !== null
                  ...
                  <dd>
                    {request.curator_agreement === "discorda" ? (<span className="font-medium text-destructive">
                        Divergência — curador: {request.curator_classification ?? "—"} · IA:{" "}
                        {request.ai_classification ?? "—"}
                      </span>) : request.curator_agreement === "concorda" ? ("Curador concordou com a IA") : ("Sem resposta do curador")}
                  </dd>
              ...
              <section className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea id="curation-admin-notes" rows={5} value={adminNotes} onChange={(event) => setAdminNotes(event.target.value)} placeholder="Anotações internas sobre a decisão, encaminhamento ou ajuste necessário."/>
                </div>
            ...
            <CurationConversationDialog requestId={conversationOpen ? request.id : null} open={conversationOpen} onOpenChange={setConversationOpen}/>
          </>)}
      </SheetContent>
    </Sheet>);
}
