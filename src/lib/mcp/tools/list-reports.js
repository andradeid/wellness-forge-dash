...
export default defineTool({
    ...
    handler: async (input, ctx) => {
        try {
            ...
            if (input.ai_status)
                query = query.eq("ai_status", input.ai_status);
            if (input.created_from)
                query = query.gte("created_at", input.created_from);
            if (input.created_to)
                query = query.lte("created_at", input.created_to);
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify({ total_retornado: reports.length, acesso: caller.isSuperAdmin ? "super_admin" : "curator", reports }, null, 2),
                    },
                ],
