...
function ChatPage() {
  ...
  // Antes abríamos o popover de tarefas automaticamente em chat novo. Isso
  // sofria de corrida de estados e às vezes não abria. Substituímos por um
  // grid inline de tarefas no corpo do chat vazio (ver bloco de render), então
  // o auto-open aqui foi desativado. O ref é mantido só para eventual retorno.
  const autoOpenedTaskRef = useRef<string | null>(null);
  void autoOpenedTaskRef;
  ...
  const startNewChatWithTask = useCallback(async (taskKey: string) => {
    ...
    if (!currentAgent?.is_super_agent) {
      toast.error("Perfil do paciente ainda não está pronto. Tente novamente em instantes.");
      return;
    }
    setNewChatPickerOpen(false);
    if (messages.length > 0) {
      await resetChat({ agentType: currentAgent.agent_id, selectedTask: taskKey } as any);
    } else {
      setSelectedTask(taskKey);
    }
  }, [agentType, agents, messages.length, resetChat, setSelectedTask]);
