...
import { ChevronRight, ImageIcon, Inbox, Loader2, Plus, Search, Sparkles, LifeBuoy, CheckCircle2, Clock, } from "lucide-react";
...
import { CurationRequestForm, CLASSIFICATION_LABELS, DIMENSION_LABELS, ATTACHMENT_BUCKET, } from "@/components/curadoria/CurationRequestForm";
...
function useSignedAttachment(path) {
    return useQuery({
...
function AttachmentThumbnail({ path }) {
    ...
    return (<img src={data} alt="Miniatura da imagem anexada à solicitação" loading="lazy" className="h-10 w-10 shrink-0 rounded-md border object-cover"/>);
}
function AttachmentPreview({ path }) {
    const { data, isLoading } = useSignedAttachment(path);
    if (isLoading) {
        return (<div className="flex h-40 items-center justify-center rounded-md border">
    ...
    return (<a href={data} target="_blank" rel="noreferrer" title="Abrir imagem em nova aba">
      <img src={data} alt="Imagem anexada à solicitação" className="max-h-80 w-full rounded-md border object-contain transition-opacity hover:opacity-90"/>
    </a>);
}
...
function DetailRow({ label, value }) {
    return (<div className="grid grid-cols-[minmax(0,120px)_minmax(0,1fr)] gap-2 py-1.5 text-sm">
...
function CuradoriaPage() {
    ...
    const listQuery = useQuery({
        ...
        queryFn: async () => {
            const { data, error } = await supabase
                ...
                .select("id, numero_sequencial, title, description, curator_classification, curator_dimension, status, created_at, updated_at, image_url, chat_id, message_id, patient_id, agent_key, ai_classification, ai_justification, ai_status, curator_agreement, duplicate_of")
    ...
    const filtered = useMemo(() => {
        ...
        return rows.filter((row) => {
            if (statusFilter === "abertas" && !OPEN_STATUSES.includes(row.status))
                return false;
            if (statusFilter === "concluidas" && !DONE_STATUSES.includes(row.status))
                return false;
            if (!term)
                return true;
    ...
    return (<div className="max-w-5xl space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <StatCard icon={LifeBuoy} label="Suporte" value={stats.suporte} hint={`${stats.melhoria} de melhoria · ${stats.comImagem} com imagem`}/>
      </div>
      ...
      <Card>
        ...
        ].map(([value, label]) => (<button key={value} type="button" onClick={() => setStatusFilter(value)} className={statusFilter === value
        ...
        <CardContent>
          {listQuery.isLoading ? (<div className="text-sm text-muted-foreground">Carregando solicitações...</div>) : listQuery.isError ? (<div className="text-sm text-destructive">
              Não foi possível carregar suas solicitações.
            </div>) : filtered.length === 0 ? (<div className="rounded-lg border border-dashed p-8 text-center text-sm text-muted-foreground">
              {rows.length === 0
                ? "Você ainda não registrou nenhuma solicitação."
                : "Nenhuma solicitação encontrada com os filtros atuais."}
            </div>) : (<ul className="divide-y rounded-lg border">
              {filtered.map((row) => (<li key={row.id}>
                  <button type="button" onClick={() => setSelected(row)} className="grid w-full grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 px-3 py-3 text-left transition-colors hover:bg-muted/60">
                    {row.image_url ? (<AttachmentThumbnail path={row.image_url}/>) : (<div className="grid h-10 w-10 shrink-0 place-items-center rounded-md border bg-muted/40">
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
          ...
          <CurationRequestForm idPrefix="curation-dialog" onSuccess={() => setFormOpen(false)}/>
        </DialogContent>
      </Dialog>
          ...
          {selected ? (<>
              ...
              <div className="mt-4 space-y-4">
                ...
                <div>
                  ...
                  <DetailRow label="Classificação" value={selected.ai_status === "done"
                ? (AI_CLASSIFICATION_LABELS[selected.ai_classification ?? ""] ??
                    selected.ai_classification)
                : "Em análise"}/>
                  {selected.ai_justification ? (<DetailRow label="Justificativa" value={selected.ai_justification}/>) : null}
                  {selected.curator_agreement ? (<DetailRow label="Sua resposta" value={selected.curator_agreement === "concorda"
                    ? "Você concordou com a classificação"
                    : "Você registrou divergência"}/>) : null}
                  {selected.duplicate_of ? (<DetailRow label="Duplicata" value={`Vinculada à solicitação #${selected.duplicate_of.slice(0, 8)}`}/>) : null}
                </div>
                ...
                <div>
                  ...
                  <DetailRow label="Situação" value={STATUS_LABELS[selected.status] ?? selected.status}/>
                  <DetailRow label="Criada em" value={formatDateTime(selected.created_at)}/>
                  {selected.updated_at ? (<DetailRow label="Atualizada em" value={formatDateTime(selected.updated_at)}/>) : null}
                  {selected.chat_id ? (<DetailRow label="Origem" value="Reportada a partir de uma conversa"/>) : (<DetailRow label="Origem" value="Solicitação avulsa"/>)}
                  {selected.agent_key ? (<DetailRow label="Agente" value={selected.agent_key}/>) : null}
                </div>
              </div>
            </>) : null}
        </SheetContent>
