...
/**
 * Criação de solicitações de curadoria + classificação por IA.
 *
 * Garantias:
 * - O report é gravado ANTES de qualquer chamada à OpenAI. Se a IA falhar,
 *   o report permanece com `ai_status = 'failed'` e campos ai_* vazios.
 * - A resposta devolvida ao curador NUNCA contém `ai_technical_direction`.
 * - Insert usa o cliente do usuário (RLS decide quem pode criar).
 */
...
export const createCurationRequest = createServerFn({ method: "POST" })
  ...
  .inputValidator((data: CreateInput) => {
    ...
    if (title.length < 3) throw new Response("O título precisa ter ao menos 3 caracteres.", { status: 400 });
    if (title.length > 200) throw new Response("Título muito longo.", { status: 400 });
    if (description.length < 10)
      throw new Response("A descrição precisa ter ao menos 10 caracteres.", { status: 400 });
    if (description.length > 8000) throw new Response("Descrição muito longa.", { status: 400 });
    ...
    const imagePath =
      typeof data?.imagePath === "string" && /^[\w./-]{1,200}$/.test(data.imagePath)
        ? data.imagePath
        : null;
    ...
    return {
      ...
      agentKey:
        typeof data?.agentKey === "string" && data.agentKey.length <= 120 ? data.agentKey : null,
    };
  })
...
export const resolveCurationDuplicate = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((data: { requestId: string; duplicateOf: string; isSame: boolean }) => {
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: own } = await supabaseAdmin
      ...
      .maybeSingle();
    if (!own || (own as any).created_by !== context.userId) {
      throw new Response("Forbidden", { status: 403 });
    }

    if (!data.isSame) return { linked: false, original: null };
    ...
    const { data: original } = await supabaseAdmin
      ...
      .maybeSingle();
    if (!original) throw new Response("Solicitação original não encontrada.", { status: 404 });

    await supabaseAdmin
      .from("curation_requests" as never)
      .update({ duplicate_of: data.duplicateOf, status: "duplicada" } as never)
      .eq("id", data.requestId);

    return {
      linked: true,
      original: {
        id: (original as any).id as string,
        title: (original as any).title as string,
        status: (original as any).status as string,
      },
    };
  });
...
export const setCuratorAgreement = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((data: { requestId: string; agrees: boolean }) => {
    if (!UUID_RE.test(String(data?.requestId ?? "")))
      throw new Response("Identificador inválido.", { status: 400 });
  ...
  .handler(async ({ context, data }) => {
    ...
    await supabaseAdmin
      .from("curation_requests" as never)
      .update({ curator_agreement: data.agrees ? "concorda" : "discorda" } as never)
      .eq("id", data.requestId);
