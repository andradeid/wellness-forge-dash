...
function supabaseProjectUrl(): string {
  ...
  if (!url) throw new Error("SUPABASE_URL (ou VITE_SUPABASE_URL) não está configurada.");
  return url;
}
...
function supabasePublishableKey(): string {
  ...
  if (!key) throw new Error("SUPABASE_PUBLISHABLE_KEY (ou SUPABASE_ANON_KEY) não está configurada.");
  return key;
}
