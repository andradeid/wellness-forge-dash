                ...
                <TooltipProvider delayDuration={150}>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div
                        className="inline-flex items-center gap-1.5 rounded-full bg-white/80 backdrop-blur-sm border border-[#e8a04c]/30 px-3 py-1 text-[11px] font-medium text-foreground/70 shadow-sm cursor-not-allowed select-none"
                        aria-disabled="true"
                      >
                        {(() => {
                          const agent = AGENT_OPTIONS.find(a => a.id === "research") || AGENT_OPTIONS[0];
                          const Icon = agent.icon;
                          return <Icon className="h-3.5 w-3.5 text-[#e8a04c]" />;
                        })()}
                        <span>{getActiveAgentLabel(agentType)}</span>
                        <span className="text-muted-foreground/60 text-[10px]">• bloqueado</span>
                      </div>
                    </TooltipTrigger>
                    <TooltipContent side="top" className="max-w-[240px] text-xs">
                      A Pesquisa Científica é uma conversa dedicada. Para usar outro agente, inicie um novo chat.
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
              ) : (
                <Popover>
                  ...
                  >
                    <div className="space-y-1">
                      {AGENT_OPTIONS.filter(opt => opt.id !== "research").map((opt, idx, arr) => {
                        const Icon = opt.icon;
